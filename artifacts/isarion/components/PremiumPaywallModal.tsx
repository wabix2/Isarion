...
import {
  ...
  purchasePackage,
  restorePurchases,
  type IsarionOffering,
} from "@/utils/premium";
import { FREE_LIMITS } from "@/utils/access";

const FEATURES = [
  "Everything in Free",
  `Unlimited mentor sessions (Free: ${FREE_LIMITS.feynman}/day)`,
  `Unlimited AI chat (Free: ${FREE_LIMITS.aiChat}/day)`,
  "Predictive weak-area detection",
  "Deeper learner-model evidence",
  `Higher AI usage limits (Free: ${FREE_LIMITS.aiChat}/day)`,
];
...
interface Props {
  ...
  /** Overrides the hero title/subtitle for context-specific framing — e.g.
   *  a win-moment paywall shown after a level-up or streak milestone reads
   *  "unlock more of this" rather than the default limit-hit framing. */
  title?: string;
  subtitle?: string;
}
...
export default function PremiumPaywallModal({
  ...
  const [offering, setOffering] = useState<IsarionOffering | null>(null);
  const [loadingOffering, setLoadingOffering] = useState(false);
  const [purchasing, setPurchasing] = useState(false);
  const [restoring, setRestoring] = useState(false);

  const [isInitialized, setIsInitialized] = useState(isPurchasesInitialized());
  ...
  useEffect(() => {
    ...
    initializePurchases()
      .catch(() => {})
      .finally(() => {
        if (active) setIsInitialized(isPurchasesInitialized());
      });
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    ...
    getOffering()
      .then((next) => {
        setOffering(next);
        setPlan(next.yearly ? "yearly" : next.monthly ? "monthly" : "lifetime");
      })
      .catch(() => {})
  ...
  const priceFor = (key: PlanKey): string => packageForPlan(key)?.product?.priceString ?? "";

  const selectedPackage = packageForPlan(plan);
  const canPurchase = isInitialized && !!selectedPackage && !purchasing && !restoring;

  const planMeta: Record<PlanKey, { label: string; badge?: string; featured?: boolean; per: string }> = {
    monthly: { label: "Monthly", per: "Billed monthly · cancel anytime" },
    yearly: { label: "Annual", badge: "Recommended", featured: true, per: "Billed annually" },
    lifetime: { label: "Lifetime", badge: "One-time", per: "Single purchase" },
  };
  ...
  async function handleRestore() {
    ...
    try {
      ...
      if (success) {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        Alert.alert("Purchases restored", "Your Isarion Pro access has been restored.", [
  ...
  const PLANS: { key: PlanKey; package: PurchasesPackage }[] = [
    offering?.monthly ? { key: "monthly", package: offering.monthly } : null,
    offering?.yearly ? { key: "yearly", package: offering.yearly } : null,
    offering?.lifetime ? { key: "lifetime", package: offering.lifetime } : null,
  ].filter((item): item is { key: PlanKey; package: PurchasesPackage } => item !== null);
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        >
          ...
          <Text style={[styles.heroTitle, { fontFamily: "Inter_700Bold" }]}>{title ?? "Isarion Pro"}</Text>
          <Text style={[styles.heroSub, { fontFamily: "Inter_400Regular" }]}>
            {subtitle ?? "Unlimited mentor sessions, deeper analytics, and a calmer path to mastery."}
          </Text>
        </LinearGradient>
        ...
        <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
          ...
          {loadingOffering ? (
            ...
            <View style={styles.plansCol}>
              {PLANS.length === 0 ? (
                <View style={[styles.noPlans, { backgroundColor: colors.card, borderColor: colors.border }]}>
                  <Ionicons name="cloud-offline-outline" size={20} color={colors.textMuted} />
                  <Text style={[styles.noPlansText, { color: colors.textSecondary, fontFamily: "Inter_400Regular" }]}>
                    Plans are temporarily unavailable. Please try again when the store connection is restored.
                  </Text>
                </View>
              ) : PLANS.map(({ key }) => {
                const { label, badge, featured, per } = planMeta[key];
          ...
          <View style={[styles.freeNote, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <Text style={[styles.freeNoteTxt, { color: colors.textSecondary, fontFamily: "Inter_400Regular" }]}>
              Free plan includes {FREE_LIMITS.feynman} Feynman sessions per day, quizzes, flashcards, and leaderboard access.
            </Text>
          </View>
          ...
          {selectedPackage && (
            <Text style={[styles.legal, { color: colors.textMuted, fontFamily: "Inter_400Regular" }]}>
              {plan === "lifetime"
                ? `One-time purchase at ${selectedPackage.product.priceString}. No recurring charges.`
                : `${selectedPackage.product.priceString} billed ${plan === "yearly" ? "annually" : "monthly"}. Auto-renews until cancelled in the store.`}
            </Text>
          )}
        </ScrollView>
      </View>
...
const styles = StyleSheet.create({
  ...
  plansCol: { gap: 10 },
  noPlans: { flexDirection: "row", alignItems: "center", gap: 10, padding: 16, borderRadius: 18, borderWidth: 1 },
  noPlansText: { flex: 1, fontSize: 13, lineHeight: 19 },

...
export default function AchievementShareModal({ achievement, userName, userLevel, referralCode, onClose }: Props) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const result = await shareCardAsImage(shotRef, {
        fallbackMessage: `I unlocked "${achievement.label}" on ${APP_NAME}. Level ${userLevel} — better study habits, one session at a time.`,
        fallbackUrl: inviteLink,
        dialogTitle: `${achievement.label} on ${APP_NAME}`,
      });
      if (result === "cancelled") {
        ...
        <Pressable style={[styles.sheet, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <Pressable
            ...
            onPress={() => {
              ...
              Linking.openURL(getInviteLink(referralCode));
            }}
          >
